...
treedata <- read_excel(paste0(directory, "/Data_for_analysis/Treemap_OutcomeBasedEffects.xlsx"))
```
...
domains_k <- treedata %>% group_by(Domain) %>% summarise(count_experiments = n()) # Experiment counts grouped by domain
...
# save as html
htmlwidgets::saveWidget(as_widget(fig), paste0(directory, "/Results/FigureOutputs/Treemap_OutcomeBasedEffects.html"))
```

``` {saving, include = FALSE}
# save as svg
# orca(fig, paste0(directory,"/Results/FigureOutputs/Treemap_OutcomeBasedEffects.svg"), width = 7.2*144, height = 5.9*144)
save_image(fig, paste0(directory,"/Results/FigureOutputs/Treemap_OutcomeBasedEffects.svg"), width = 7.2*144, height = 5.9*144)
